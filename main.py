    ...
    if user_id in lead_progress:
        ...
        if stage == "platform":
            ...
            if is_meaningful_reply("Укажите, пожалуйста, номер WhatsApp, по которому с вами можно связаться.", text):
                lead["contact"] = text
                lead["stage"] = "name"
                send_telegram_message(chat_id, "Отлично! Как мне вас называть?")
                return "ok"
            else:
            ...
            if is_meaningful_reply("Как к вам можно обращаться?", text):
                lead["name"] = text
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)
            ...
            for project in ["OM", "TAO", "BUDDHA"]:
                ...
                # Если это не ответ, обрабатываем сообщение GPT-логикой
                sessions[user_id] = sessions.get(user_id, [])
sk, request
import openai
import requests
